...
use rusty_core::reports::{self, ChangeType};
...
#[test]
fn change_summary_log_line_counts_rescan_changes() {
    let changes = vec![
        reports::FileChange {
            change_type: ChangeType::Moved,
            hash: "a".into(),
            prev_path: "/old/a".into(),
            new_path: "/new/a".into(),
            file_name: "a".into(),
            size: 1,
    ...
    assert_eq!(
        rusty_core::commands::change_summary_log_line(&changes),
        "rescan change summary — moved: 1, renamed: 1, changed: 1, new: 1, gone: 1"
    );
}
...
#[test]
fn scan_diff_classifies_move_rename_change_new_and_gone() {
    let temp = tempfile::tempdir().unwrap();
    let src = temp.path().join("src");
    fs::create_dir_all(src.join("old")).unwrap();
    fs::write(src.join("old/moved.txt"), b"moved body same name").unwrap();
    fs::write(src.join("old/renamed.txt"), b"renamed body").unwrap();
    fs::write(src.join("old/changed.txt"), b"changed before").unwrap();
    fs::write(src.join("old/gone.txt"), b"gone body").unwrap();
    ...
    let scan_id = memory
        .start_scan("dry", &[src.to_string_lossy().into()])
        .unwrap();
    let (first_files, first_counters) =
        scanner::scan_roots(&[src.clone()], &memory, &logs, &cancel, scan_id).unwrap();
    let first_report = dedupe::group_duplicates(&first_files);
    memory
        .finish_scan(
            scan_id,
            first_counters.files_walked as i64,
            first_counters.bytes_walked as i64,
            first_counters.hashes_reused as i64,
            first_counters.hashes_computed as i64,
            first_report.groups.len() as i64,
            first_report.total_wasted_bytes,
        )
        .unwrap();

    fs::create_dir_all(src.join("new")).unwrap();
    fs::rename(src.join("old/moved.txt"), src.join("new/moved.txt")).unwrap();
    fs::rename(src.join("old/renamed.txt"), src.join("old/renamed-now.txt")).unwrap();
    fs::write(src.join("old/changed.txt"), b"changed after").unwrap();
    fs::write(src.join("new/brand-new.txt"), b"brand new").unwrap();
    fs::remove_file(src.join("old/gone.txt")).unwrap();

    let scan_id2 = memory
        .start_scan("dry", &[src.to_string_lossy().into()])
        .unwrap();
    let (second_files, second_counters) =
        scanner::scan_roots(&[src.clone()], &memory, &logs, &cancel, scan_id2).unwrap();
    let second_report = dedupe::group_duplicates(&second_files);
    memory
        .finish_scan(
            scan_id2,
            second_counters.files_walked as i64,
            second_counters.bytes_walked as i64,
            second_counters.hashes_reused as i64,
            second_counters.hashes_computed as i64,
            second_report.groups.len() as i64,
            second_report.total_wasted_bytes,
        )
        .unwrap();

    let changes = memory.scan_changes(scan_id2).unwrap();
    let changed_paths: Vec<_> = changes
        .iter()
        .map(|c| (c.change_type, c.prev_path.as_str(), c.new_path.as_str()))
        .collect();

    assert!(
        changed_paths.iter().any(|(t, prev, new)| {
            *t == ChangeType::Moved
                && prev.ends_with("/old/moved.txt")
                && new.ends_with("/new/moved.txt")
        }),
        "expected MOVED entry, got {changed_paths:?}"
    );
    assert!(
        changed_paths.iter().any(|(t, prev, new)| {
            *t == ChangeType::Renamed
                && prev.ends_with("/old/renamed.txt")
                && new.ends_with("/old/renamed-now.txt")
        }),
        "expected RENAMED entry, got {changed_paths:?}"
    );
    assert!(
        changed_paths.iter().any(|(t, prev, new)| {
            *t == ChangeType::Changed
                && prev.ends_with("/old/changed.txt")
                && new.ends_with("/old/changed.txt")
        }),
        "expected CHANGED entry, got {changed_paths:?}"
    );
    assert!(
        changed_paths.iter().any(|(t, prev, new)| {
            *t == ChangeType::New && prev.is_empty() && new.ends_with("/new/brand-new.txt")
        }),
        "expected NEW entry, got {changed_paths:?}"
    );
    assert!(
        changed_paths.iter().any(|(t, prev, new)| {
            *t == ChangeType::Gone && prev.ends_with("/old/gone.txt") && new.is_empty()
        }),
        "expected GONE entry, got {changed_paths:?}"
    );
}

#[test]
fn manifests_include_all_files_and_folder_fingerprints() {
    let temp = tempfile::tempdir().unwrap();
    let src = temp.path().join("src");
    fs::create_dir_all(src.join("album/nested")).unwrap();
    fs::write(src.join("album/a.txt"), b"alpha").unwrap();
    fs::write(src.join("album/b.txt"), b"beta").unwrap();
    fs::write(src.join("album/nested/c.txt"), b"gamma").unwrap();
    ...
    assert_eq!(manifests.files.len(), 3);
    assert!(manifests
        .files
        .iter()
        .all(|f| !f.path.is_empty() && !f.content_hash.is_empty() && f.size > 0));

    let album = manifests
        .folders
        .iter()
        .find(|f| f.path.ends_with("/album"))
        .expect("album folder manifest");
    assert_eq!(
        album.file_count, 2,
        "folder fingerprint counts direct files only"
    );
    assert_eq!(album.total_bytes, 9);
    assert!(!album.folder_hash.is_empty());

    let nested = manifests
        .folders
        .iter()
        .find(|f| f.path.ends_with("/album/nested"))
        .expect("nested folder manifest");
    assert_eq!(nested.file_count, 1);
    assert_eq!(nested.total_bytes, 5);
    assert!(!nested.folder_hash.is_empty());
}
