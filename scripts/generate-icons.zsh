...
# If a custom designed high-res ICNS exists in the repo root, override the generated files
if [[ -f "$repo_root/rusty.icns" ]]; then
    print "Overriding generated ICNS with custom icon from $repo_root/rusty.icns"
    cp -f "$repo_root/rusty.icns" "$icons_dir/icon.icns"
    cp -f "$repo_root/rusty.icns" "$icons_dir/Rusty.icns"
else
